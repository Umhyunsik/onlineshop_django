...
class Order(models.Model):
    ...
    discount=models.IntegerField(default=0,validators=[MinValueValidator(0),MaxValueValidator(10000)])
...
class OrderItem(models.Model):
    ...
    #product 입장에서 내가 어디에 주문이 들어갔는지 그 order 뭐있니
    price = models.DecimalField(max_digits=10,decimal_places=2)#소숫점 2자리
...
class OrderTransactionManager(models.Manager):
    def create_new(self, order, amount, success=None, transaction_status=None):
        ...
        #print(transaction.order,transaction.merchant_order_id,transaction.amount,"거래정")
...
#         if not order:
#             print("주문정보오류")
#             raise ValueError("주문 정보오류 ")
#         order_hash=hashlib.sha1(str(order.id).encode('utf-8')).hexdigest()
#         emali_hash=str(order.email).split("@")[0]
#         final_hash=hashlib.sha1((order_hash+emali_hash.encode('utf-8'))).hexdigest()[:10]
#         merchant_order_id= str(final_hash)
#         payments_prepare(merchant_order_id,amount)
#         transaction = self.model(
#             order=order,
...
#sender가 보내면 order_payment_validation할꺼